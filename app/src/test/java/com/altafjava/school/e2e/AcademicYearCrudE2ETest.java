...
class AcademicYearCrudE2ETest extends SchoolIntegrationTestBase {
	...
	void setup() {
		...
		Tenant tenant = onboardingService.registerTenant(new RegisterTenantCommand(
				"AcademicYear E2E School", "ay-e2e-" + suffix, 1L, adminEmail, adminPassword, "USD"));
		tenantId = tenant.getId();
	}

	@Test
	void createAcademicYear_asTenantAdmin_returns201() {
	...
	@Test
	void listAcademicYears_withoutJwt_returns401() {
		given()
				.header("X-Tenant-ID", tenantId)
				.contentType(ContentType.JSON)
				.when()
	...
	@Test
	void createAcademicYear_asTeacherRole_returns403() {
	...
	@Test
	void academicYearCreatedUnderOneTenant_returns404ForAnotherTenant() {
		String accessToken = login();
		...
		Tenant otherTenant = onboardingService.registerTenant(new RegisterTenantCommand(
				"Other School", "ay-other-" + otherSuffix, 1L,
