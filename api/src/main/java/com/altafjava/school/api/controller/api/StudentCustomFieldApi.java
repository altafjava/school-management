...
@Tag(name = "Student Custom Field", description = "APIs for managing Student Custom Field operations.\n\n**Tenant Scope**: All endpoints are tenant-scoped via X-Tenant-ID header.\n**Auth**: JWT Bearer token required on all endpoints unless marked public.")
@SecurityRequirement(name = "bearerAuth")
@SecurityRequirement(name = "tenantHeader")
public interface StudentCustomFieldApi {

	@Operation(summary = "Get", operationId = "studentcustomfield_get")
	public ApiResponse<List<CustomFieldValueResponse>> get(@PathVariable String publicId);

	@Operation(summary = "Get grouped", operationId = "studentcustomfield_getGrouped")
	public ApiResponse<List<FieldGroupResponse>> getGrouped(@PathVariable String publicId);

	@Operation(summary = "Set", operationId = "studentcustomfield_set")
