...
@Tag(name = "Guardian Custom Field", description = "APIs for managing Guardian Custom Field operations.\n\n**Tenant Scope**: All endpoints are tenant-scoped via X-Tenant-ID header.\n**Auth**: JWT Bearer token required on all endpoints unless marked public.")
@SecurityRequirement(name = "bearerAuth")
@SecurityRequirement(name = "tenantHeader")
public interface GuardianCustomFieldApi {

	@Operation(summary = "Get", operationId = "guardiancustomfield_get")
	public ApiResponse<List<CustomFieldValueResponse>> get(@PathVariable String publicId);

	@Operation(summary = "Get grouped", operationId = "guardiancustomfield_getGrouped")
	public ApiResponse<List<FieldGroupResponse>> getGrouped(@PathVariable String publicId);

	@Operation(summary = "Set", operationId = "guardiancustomfield_set")
