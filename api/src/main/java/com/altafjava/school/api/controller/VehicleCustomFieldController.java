...
import com.altafjava.school.api.controller.api.VehicleCustomFieldApi;
...
// A clearly-scoped sibling to VehicleController rather than an extension of it — see
// StudentCustomFieldController's own Javadoc for the same rationale.
@RestController
@RequestMapping("/api/v1/vehicles/{publicId}/custom-fields")
public class VehicleCustomFieldController implements VehicleCustomFieldApi {

	private final VehicleService vehicleService;
	private final CustomFieldValueService customFieldValueService;
	private final CustomFieldValueMapper customFieldValueMapper;

	public VehicleCustomFieldController(VehicleService vehicleService, CustomFieldValueService customFieldValueService,
	...
	public ApiResponse<List<CustomFieldValueResponse>> get(@PathVariable String publicId) {
		...
		return ApiResponse.success(customFieldValueMapper.toResponseList(
				customFieldValueService.getAllValues(CustomFieldEntityType.TRANSPORT_VEHICLE, vehicle.getId())));
	}
	...
	public ApiResponse<List<CustomFieldValueResponse>> set(@PathVariable String publicId,
		...
		for (Map.Entry<String, String> entry : request.values().entrySet()) {
			customFieldValueService.setValue(CustomFieldEntityType.TRANSPORT_VEHICLE, vehicle.getId(), entry.getKey(),
					entry.getValue());
		}
		return ApiResponse.success(customFieldValueMapper.toResponseList(
				customFieldValueService.getAllValues(CustomFieldEntityType.TRANSPORT_VEHICLE, vehicle.getId())));
