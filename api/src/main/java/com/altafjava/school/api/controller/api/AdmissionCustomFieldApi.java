...
@Tag(name = "Admission Custom Field", description = "APIs for managing Admission Custom Field operations.\n\n**Tenant Scope**: All endpoints are tenant-scoped via X-Tenant-ID header.\n**Auth**: JWT Bearer token required on all endpoints unless marked public.")
@SecurityRequirement(name = "bearerAuth")
@SecurityRequirement(name = "tenantHeader")
public interface AdmissionCustomFieldApi {

	@Operation(summary = "Get", operationId = "admissioncustomfield_get")
	public ApiResponse<List<CustomFieldValueResponse>> get(@PathVariable String publicId);

	@Operation(summary = "Get grouped", operationId = "admissioncustomfield_getGrouped")
	public ApiResponse<List<FieldGroupResponse>> getGrouped(@PathVariable String publicId);

	@Operation(summary = "Set", operationId = "admissioncustomfield_set")
