...
- `Guardian`/`Parent` entity + relationship to `Student`. This is the single biggest missing piece — almost every real school workflow (fee responsibility, communication, consent, report access) assumes a guardian exists, and retrofitting a third actor type after building parent-facing features around it is expensive. Model it now, before more is built on `Student` alone.
- Grade computation: numeric → letter grade, with the grading scale itself tenant-configurable (via `TenantSettingOverride`, already exists platform-side — don't hardcode a single scale).
- Fee balance / outstanding / partial-payment tracking on `FeePayment`+`FeeStructure` — right now a payment is recorded but nothing ever computes what's owed. A fee module that can't answer "how much does this student still owe" isn't a fee module yet.
...
| **Guardians** *(new)* | Guardian identity, guardian↔student links, consent | `Guardian`, `StudentGuardianLink` | Student (by ID) | `GuardianLinkedEvent` |
...
**Phase 1 — Make the core academic + fee workflow real (~3–4 weeks).** Guardian entity + student links, grade computation with tenant-configurable scale, fee balance/partial-payment tracking. **Testing/CI work (`TESTING.md` §5, steps 5–7):** JaCoCo ratchet gate goes blocking (floor = Phase 0's recorded baseline), `:domain:pitest` wired into CI and blocking at `mutationThreshold = 70`, spotbugs added with a zero-findings baseline, full RBAC E2E matrix (§1 table) for every Guardian/Student-facing endpoint. **Dependencies:** Phase 0 (roles must exist before guardian-scoped access control makes sense). **Outcome:** a parent (once a frontend exists) could genuinely see their child's real grade and real fee balance — the numbers would be correct, not placeholder. **Validate:** integration tests proving a guardian can only see their own linked student's data; mutation score on `GradeCalculation`/`FeeBalanceCalculator` meets threshold, not just line coverage.
...
1. **Build first:** ~~Phase 0's correctness fixes~~ — done (2026-08-16). Build Phase 1 (Guardian, real grade/fee logic) next — see §2/§7.
